...
public final class ConfigurationManager<C> {
	private final Logger logger;
	private final ConfigurationHelper<C> helper;
	private volatile C data;

	private ConfigurationManager(ConfigurationHelper<C> configurationHelper, Logger logger) {
		this.helper = configurationHelper;
		this.logger = logger;
	}

	public static <C> ConfigurationManager<C> create(Path configurationFolder, String fileName, Class<C> configurationClass, Logger logger) {
		// SnakeYaml example
		...
		return new ConfigurationManager<>(new ConfigurationHelper<>(configurationFolder, fileName, factory), logger);
	}
